...
**D) Determine el valor esperado de la predicción $(E(Y))$ para $z_{1} = 6$ y $z_{2} = 4$. Calcule su intervalo de confianza del $95\%$ correspondiente (el valor esperado).**

```{r}
# vector de input para hacer la predicción y estimación del intervalo
z0 = rbind(1, 6, 4)

# valor de la predicción dado z0
prediction = t(z0)%*%(betas)
prediction
```
...
get_interval_for_y <- function(betas, z, z0, y, alpha){
  
  # Dados:
  # betas: maximum likelihood estimator para los betas (vector columna)
  # z: matriz de datos donde cada columna representa una variable distinta
  # z0: datos input para predecir la función y (vector columna)
  # y: (vector columna) de las observaciones
  # alpha: nivel de confianza
  
  # Retorna:
  # intervalos de confianza superior e inferior para
  # el valor esperado de Y, dado z0 como input
  ...
  # sum of squares of deviations, entre lo observado 
  # y lo calculado
  sum_of_squared_errors = get_error(betas, z, y)
  
  # s^2
  s_squared = sum_of_squared_errors/(ndatapoints - nvars -1)
  
  prediction = t(z0) %*% betas
  amplitude = qt(p = 1-alpha, df = (ndatapoints - nvars -1))
  amplitude = amplitude * sqrt( (t(z0) %*% solve(t(z)%*%z) %*% z0) * s_squared)
  
  inferior = prediction - amplitude
  superior = prediction + amplitude
  
  return(data.frame(inferior, superior))
}
```

Vea el intervalo para el $E(Y | Z_{0})$:

```{r}
# vector de input para hacer la predicción y estimación del intervalo
z0 = rbind(1, 6, 4)

get_interval_for_y(betas = betas, z = z, z0 = z0, y = y, alpha = 0.05)
```

**E) Determine el intervalo de confianza del $95\%$ para la predicción $(Y)$ cuando $z_{1} = 6$ y $z_{2} = 4$**
